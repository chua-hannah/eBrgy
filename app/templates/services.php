<section class="section-padding">
    <div class="container">
        <div class="row">
            <div class="col-lg-10 col-12 text-center mx-auto">
                <h2 class="mt-4 mb-4">Services</h2>
            </div>

            <div class="col-lg-3 col-md-6 col-12 mb-4 mb-lg-0">
                <div class="featured-block d-flex justify-content-center align-items-center">
                    <a href="/eBrgy/app/documents" class="d-block">
                        <img src="assets/images/icons/stamp.png" class="featured-block-image img-fluid" alt="">

                        <p class="featured-block-text"><strong>Request a Document</strong></p>
                    </a>
                </div>
            </div>

            <div class="col-lg-3 col-md-6 col-12 mb-4 mb-lg-0 mb-md-4">
                <div class="featured-block d-flex justify-content-center align-items-center">
                    <a href="/eBrgy/app/reports" class="d-block">
                        <img src="assets/images/icons/letter.png" class="featured-block-image img-fluid" alt="">

                        <p class="featured-block-text"><strong>Submit a Complaint or Report</strong></p>
                    </a>
                </div>
            </div>

            <div class="col-lg-3 col-md-6 col-12 mb-4 mb-lg-0 mb-md-4">
                <div class="featured-block d-flex justify-content-center align-items-center">
                    <a href="/eBrgy/app/equipments" class="d-block">
                        <img src="assets/images/icons/chair.png" class="featured-block-image img-fluid" alt="">

                        <p class="featured-block-text"><strong>Borrow Materials</strong></p>
                    </a>
                </div>
            </div>

            <div class="col-lg-3 col-md-6 col-12 mb-4 mb-lg-0 mb-md-4">
                <div class="featured-block d-flex justify-content-center align-items-center">

                    <a href="/eBrgy/app/schedules" class="d-block">
                        <img src="assets/images/icons/bb.png" class="featured-block-image" alt="" style="height: 120px;">
                        <p class="featured-block-text"><strong>Book a Schedule</strong></p>
                    </a>
                </div>
            </div>
        </div>
    </div>
</section>